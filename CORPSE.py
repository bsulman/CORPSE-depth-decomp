...
class soil_carbon_cohort:
    def __init__(self,
            litterC=zeros((2,3)),
            protectedC=zeros((2,3)),
            livingMicrobeC=zeros((2,1)),
            CO2=zeros((2,1)),
            params=None):
        ...
        self.livingMicrobeC=array(livingMicrobeC)
        self.CO2=array(CO2)
        self.originalC=(litterC+protectedC).sum(axis=1,keepdims=True)+livingMicrobeC+CO2
    ...
    def add_carbon(self,litterC=zeros((2,3)),protectedC=zeros((2,3)),
                        livingMicrobeC=zeros((2,1)), CO2=zeros((2,1))):
        ...
        self.originalC=self.originalC+(litterC+protectedC).sum(axis=1,keepdims=True)+livingMicrobeC+CO2
    ...
    def update(self,T,theta,dt):
        ...
        # Microbial turnover
        microbeTurnover=max(0.0,self.livingMicrobeC.sum()-self.params['minMicrobeC']*self.litterC.sum())/self.params['Tmic']*self.livingMicrobeC/self.livingMicrobeC.sum()   # kg/m2/yr
        maintenance_resp=microbeTurnover*(1.0-et)

        deadmic_C_produced=dt*microbeTurnover*et   # actual fraction of microbial turnover
        self.litterC[:,2]=self.litterC[:,2]+deadmic_C_produced[:,0]  # kg/m2

        # CO2 production and cumulative CO2 produced by cohort
        CO2prod=dt*((tempResp*(1.0-eup)).sum(axis=1,keepdims=True)+maintenance_resp) # kg/m2
        self.CO2=self.CO2+CO2prod  # kg/m2

        microbeGrowth=(tempResp*eup).sum(axis=1,keepdims=True);
    ...
    def Resp(self,Ctotal,Chet,T,theta):
        ...
        theta_resp_max=self.params['substrate_diffusion_exp']/(self.params['gas_diffusion_exp']*(1.0+self.params['substrate_diffusion_exp']/self.params['gas_diffusion_exp']))
        aerobic_max=theta_resp_max**self.params['substrate_diffusion_exp']*(1.0-theta_resp_max)**self.params['gas_diffusion_exp']

        if(Ctotal.sum()==0.0 or theta==0.0 or Chet.sum()==0.0):
            return 0.0


        Resp=self.Vmax(T)*theta**self.params['substrate_diffusion_exp']*(Ctotal)*Chet.sum()/(Ctotal*self.params['kC']+Chet.sum())*(1.0-theta)**self.params['gas_diffusion_exp']/aerobic_max;
    ...
    def totalC(self,includeCO2=True):
        if includeCO2:
            return (self.litterC+self.protectedC).sum(axis=1,keepdims=True)+self.livingMicrobeC+self.CO2;
        else:
            return (self.litterC+self.protectedC).sum(axis=1,keepdims=True)+self.livingMicrobeC
    ...
    def check_validity(self,tolerance=1e-6):
        ...
        if any(self.livingMicrobeC<0) or any(self.CO2<0) or any(self.originalC<0):
            bad=True;

        if any(~isfinite(self.litterC)) or any(~isfinite(self.protectedC)) or any(~isfinite(self.livingMicrobeC)) or \
                any(~isfinite(self.CO2)) or any(~isfinite(self.originalC)):
            bad=True;
        ...
        if any(abs(self.totalC()-self.originalC) > tolerance):
            print (self)
    ...
    def __str__(self):
        s='soil_carbon_cohort(litterC=[[%1.2g,%1.2g,%1.2g],[%1.2g,%1.2g,%1.2g]], protectedC=[[%1.2g,%1.2g,%1.2g],[%1.2g,%1.2g,%1.2g]], livingMicrobeC=[%1.2g,%1.2g], CO2=[%1.2g,%1.2g], originalC=[%1.2g,%1.2g])'\
                    %(self.litterC[0,0],self.litterC[0,1],self.litterC[0,2],
                      self.litterC[1,0],self.litterC[1,1],self.litterC[1,2],
                      self.protectedC[0,0],self.protectedC[0,1],self.protectedC[0,2],
                      self.protectedC[1,0],self.protectedC[1,1],self.protectedC[1,2],
                      self.livingMicrobeC[0],self.livingMicrobeC[1],self.CO2[0],self.CO2[1],self.originalC[0],self.originalC[1])
        return s
